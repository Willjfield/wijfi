...
function splitKeyCombination(combination) {
  ...
  const flushBuffer = () => {
    if (buffer) {
      keys2.push(normalizeKey(buffer));
      buffer = "";
    }
  };
  ...
  flushBuffer();
  const hasInvalidMinus = keys2.some((key) => key.length > 1 && key.includes("-") && key !== "--");
  if (hasInvalidMinus) {
    if (!isInternal) consoleWarn(`Invalid hotkey combination: "${combination}" has invalid structure`);
    return [];
  }
  if (keys2.length === 0 && combination) {
...
function useHotkey(keys2, callback) {
  ...
  watch(() => toValue(keys2), function(unrefKeys) {
    cleanup();
    if (unrefKeys) {
...
//# sourceMappingURL=chunk-A466ZLYA.js.map
